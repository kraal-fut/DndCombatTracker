...
test('characters are ordered by initiative initially', function () {
    ...
    // Now sorted by order field: Fighter (0), Wizard (1), Rogue (2)
    expect($characters->pluck('name')->toArray())->toBe(['Fighter', 'Wizard', 'Rogue']);
    // But initiative values are preserved
...
test('can advance to next turn', function () {
    ...
    // Sorted by order first: Fighter (order 0) comes before Wizard (order 1)
    $characters = $combat->fresh()->characters;
    expect($characters->first()->name)->toBe('Fighter');
    expect($characters->first()->order)->toBe(0);
    
    // After next turn, Fighter's order increases, Wizard now has lower order so comes first
