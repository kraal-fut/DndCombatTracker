...
@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Welcome Message -->
            <div class="bg-gradient-to-r from-red-900 to-gray-800 overflow-hidden shadow-xl sm:rounded-lg mb-6 border border-gray-700">
                <div class="p-6">
                                ...
                                <dl>
                                    ...
                                    <dd class="text-3xl font-bold text-white">
                                        @php
                                            $user = Auth::user();
                                            if ($user->isAdmin()) {
                                                echo \App\Models\Combat::count();
                                            } elseif ($user->isDM()) {
                                                echo \App\Models\Combat::where('user_id', $user->id)->count();
                                            } else {
                                                echo \App\Models\Combat::whereHas('characters', function($q) use ($user) {
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-3xl font-bold text-white">
                                        @php
                                            $user = Auth::user();
                                            $query = \App\Models\Combat::where('status', 'active');
                                            
                                            if ($user->isDM()) {
                                                echo $query->where('user_id', $user->id)->count();
                                            } elseif ($user->isPlayer()) {
                                                echo $query->whereHas('characters', function($q) use ($user) {
                                                    $q->where('user_id', $user->id);
                                                })->count();
                                            } else {
                                                echo $query->count();
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-3xl font-bold text-white">{{ \App\Models\CombatCharacter::count() }}</dd>
                                </dl>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg border border-gray-700">
                <div class="p-6">
                    ...
                    @php
                        $user = Auth::user();
                        $query = \App\Models\Combat::with('characters');
                        
                        if ($user->isAdmin()) {
                            $recentCombats = $query->latest()->take(5)->get();
                        } elseif ($user->isDM()) {
                            $recentCombats = $query->where('user_id', $user->id)->latest()->take(5)->get();
                        } else {
                            $recentCombats = $query->whereHas('characters', function($q) use ($user) {
                                $q->where('user_id', $user->id);
                            })->latest()->take(5)->get();
                        }
                    @endphp
                    ...
                    @if($recentCombats->isEmpty())
                        ...
                        <div class="text-center mt-4">
                            @can('create', \App\Models\Combat::class)
                                <a href="{{ route('combats.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition">
                                    Create Combat
                                </a>
                            @endcan
                        </div>
                    @else
                        <div class="space-y-3">
                            @foreach($recentCombats as $combat)
                                <a href="{{ route('combats.show', $combat) }}" class="block p-4 bg-gray-700 hover:bg-gray-650 rounded-lg transition border border-gray-600">
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                @can('create', \App\Models\Combat::class)
                    <div class="bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg border border-gray-700 p-6">
                        ...
                        <div class="space-y-2">
                            <a href="{{ route('combats.create') }}" class="block w-full text-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition">
                                Create New Combat
                            </a>
                            <a href="{{ route('combats.index') }}" class="block w-full text-center px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white font-semibold rounded-lg transition">
                                View All Combats
                            </a>
                        </div>
                    </div>
                ...
                @if(Auth::user()->isAdmin())
                    <div class="bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg border border-gray-700 p-6">
                        ...
                        <div class="space-y-2">
                            <a href="{{ route('admin.dashboard') }}" class="block w-full text-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition">
                                Manage Users
                            </a>
